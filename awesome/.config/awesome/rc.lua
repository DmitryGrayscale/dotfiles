...
local function run_once_not_exact(cmd_arr)
    for _, cmd in ipairs(cmd_arr) do
        awful.spawn.with_shell(string.format("pgrep '%s' > /dev/null || (%s)", "mopidy", cmd))
    end
end
...
awful.util.tagnames = { "one", "two", "three", "four" }
...
globalkeys = my_table.join(
   ...
   awful.key({ modkey, "Shift" }, "s", function () awful.spawn.with_shell("maim -s --format png /dev/stdout | xclip -selection clipboard -t image/png") end,
...
--    awful.key({ altkey, }, "c", function () if beautiful.cal then beautiful.cal.show(7) end end,
    ...
    awful.key({ }, "XF86ScreenSaver", function () os.execute("if [ `xbacklight -get` == 0.000000 ]; then xbacklight -set 100; else xbacklight -set 0; fi") end,
              {description = "brightness off", group = "hotkeys"}),
    ...
    awful.key({ }, "XF86AudioMute",
        function ()
            os.execute(string.format("amixer -D pulse sset %s toggle", "Master"))
        end,
    ...
    awful.key({ modkey }, "r", function ()
        ...
        {description = "show rofi (C-Tab)", group = "launcher"}),
    -- Prompt
...
awful.rules.rules = {
    ...
    { rule = { class = "Opera" },
      properties = { screen = 1, tag = awful.util.tagnames[2], maximized = true, floating = false } },

    { rule = { class = "Gimp", role = "gimp-image-window" },
          properties = { maximized = true, tag = awful.util.tagnames[3] } },

    { rule = { class = "Wfica" },
      properties = { screen = 1, tag = awful.util.tagnames[3], maximized = true, floating = false } },
}
-- }}}
